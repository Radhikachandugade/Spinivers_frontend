...
export const getRewardsByWalletAddress =
  (walletAddress) => async (dispatch) => {
    try {
      const res = await axios.get(`spinivers_backend.railway.internal/api/rewards/${walletAddress}`);
...
export const getTodayRewardStats = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get("spinivers_backend.railway.internal/api/rewards/stats/today");
